...
class Holding::ForwardCalculatorTest < ActiveSupport::TestCase
  ...
  test "cost_basis is nil for a security whose trade has a missing FX rate" do
    ...
    today = holdings.find { |h| h.security_id == eur_security.id && h.date == Date.current }
  ...
  test "cost_basis is nil for all dates after a trade with a missing FX rate" do
    ...
    two_days_ago = holdings.find { |h| h.security_id == eur_security.id && h.date == 2.days.ago.to_date }
    ...
    [ 1.day.ago.to_date, Date.current ].each do |date|
      holding = holdings.find { |h| h.security_id == eur_security.id && h.date == date }
      assert_not_nil holding
      assert_nil holding.cost_basis, "cost_basis should be nil on #{date} after the missing-rate trade"
    end
  end
  ...
  test "missing FX stops shadowing cost basis after liquidation and repurchase" do
    load_prices

    create_trade(@voo, qty: 10, date: 4.days.ago.to_date, price: 460, account: @account, currency: "EUR")
    create_trade(@voo, qty: -10, date: 3.days.ago.to_date, price: 480, account: @account)
    create_trade(@voo, qty: 10, date: 1.day.ago.to_date, price: 490, account: @account)

    current = Holding::ForwardCalculator.new(@account).calculate.find { |h| h.security_id == @voo.id && h.date == Date.current }
